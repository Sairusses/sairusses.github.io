...
export default function FindJobsPage() {
  ...
  const handleSubmitProposal = async () => {
    ...
    try {
      await submitProposal({
        ...
        clientId: selectedJob.client, // Changed from clientId to client
        proposalText,
        coverLetter,
  ...
  const JobCard = ({ job }: { job: Job }) => {
    ...
    // Calculate display budget based on payment type
    const displayBudget = job.paymentType === "hourly" ? job.hourlyRate || 0 : job.fixedPrice || 0

    const budgetLabel = job.paymentType === "hourly" ? `/hr` : ` total`
            ...
            <div className="flex justify-between items-start">
              <div className="flex-1">
                ...
                <span className="flex items-center gap-1">
                  <DollarSign className="h-4 w-4" />${displayBudget.toLocaleString()}
                  {budgetLabel}
                </span>
                  <span className="flex items-center gap-1">
              ...
              <Dialog>
                ...
                <DialogContent className="max-w-2xl">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Posted by {job.clientName || "Unknown Client"} • ${displayBudget.toLocaleString()}
                      {budgetLabel} • {job.experienceLevel || "Any"} level
                    </DialogDescription>
                  </DialogHeader>
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">
                        {job.paymentType === "hourly"
                            ? `$${(job.hourlyRate || 0).toLocaleString()} per hour`
                            : `$${(job.fixedPrice || 0).toLocaleString()} fixed price`}
                      </p>
                    </div>
                    <div>
